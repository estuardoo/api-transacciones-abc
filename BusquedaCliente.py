...
def _parse_params(params):
    # retorna (id_value, fecha, desde, hasta)
    idv = params.get("IDCliente")
    if not idv:
        raise ValueError("Falta IDCliente")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if fecha and not (desde or hasta):
            ...
            q = table.query(
                IndexName=INDEX,
                KeyConditionExpression=Key("IDCliente").eq(idv) & Key("FechaHoraOrden").between(ini, fin),
        ...
        # Caso 3: sin fechas -> mes del último registro
        latest = table.query(
            IndexName=INDEX,
            KeyConditionExpression=Key("IDCliente").eq(idv),
            ScanIndexForward=False,
            Limit=1
        )
        ...
        # obtiene Fecha del último registro
        ult = items[0]

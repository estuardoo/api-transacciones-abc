...
def _parse_params(params):
    ...
    if not idv:
        raise ValueError("Falta IDTarjeta")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if fecha and not (desde or hasta):
            ...
            q = table.query(
                IndexName=INDEX,
                KeyConditionExpression=Key("IDTarjeta").eq(idv) & Key("FechaHoraOrden").between(ini, fin),
        ...
        latest = table.query(
            IndexName=INDEX,
            KeyConditionExpression=Key("IDTarjeta").eq(idv),
            ScanIndexForward=False,
            Limit=1
        )
